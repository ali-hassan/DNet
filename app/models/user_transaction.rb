class UserTransaction < ApplicationRecord
  ...
  after_create do |ut|
    ...
    (ut.wallet_type == 'weekly_roi_to_cash') && ut.log_and_roi_2_cash_wallet
  end
  def log_and_roi_2_cash_wallet
    user.log_histories.create(logable: self, log_type: wallet_type)
    receiver.update(
      weekly_roi_to_cash_amount: receiver.weekly_roi_to_cash_amount.try(:to_f) + amount.try(:to_f),
  ...
  def user_wallet_transafer
    { cash_wallet: :cash_wallet_total, smart_wallet: :smart_wallet_balance, weekly_roi: :current_total_weekly_roi_amount, weekly_roi_to_cash: :current_total_weekly_roi_amount }[wallet_type.try(:to_sym)]
  end
end
